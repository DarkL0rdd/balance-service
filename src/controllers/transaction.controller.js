...
export const createTransferTransaction = async (req, res, next) => {
  try {
    await transferToAnotherUser(req.body.id, req.body.amount, req.body.destination);
    res.status(200).json({ Message: `${req.body.amount}$ has been transferred to another user.` });
  } catch (error) {
    next(error);
  }
};
